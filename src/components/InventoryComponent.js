export class InventoryComponent {
    constructor(scene, x, y, width, height, items, slotSize = 100, padding = 10, backgroundColor = 0xace5ee, slotColor = 0xffffff) {
      this.scene = scene;
      this.x = x;
      this.y = y;
      this.width = width;
    ...
    createSlots() {
      ...
      for (let row = 0; row < rows; row++) {
        for (let col = 0; col < cols; col++) {
          ...
          graphics.fillRoundedRect(slotX, slotY, this.slotSize, this.slotSize, 10);
          ...
          if (itemIndex < this.items.length) {
            ...
            const itemImage = this.scene.add
              .image(slotX + this.slotSize / 2, slotY + this.slotSize / 2, item.name)
